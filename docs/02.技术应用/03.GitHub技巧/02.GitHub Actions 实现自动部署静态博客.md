...
一开始的部署方式是使用[sh部署脚本](https://github.com/954118124/vuepress-theme-vdoing/blob/master/deploy.sh)把代码提交到这两个平台的仓库分支，虽然已经很方便了，但是我还想把博客未打包的源码提交到Github主分支上。这就需要我操作两次命令，**我就想能不能只需要一次操作就可以同时把源码、部署代码一次性提交到两个平台呢？**
...
<img src="https://cdn.jsdelivr.net/gh/xugaoyi/image_store/blog/20200103124812.jpg" class="imgcss" width="70%">
...
# deploy to github
echo 'blog.luoxiaofeng.com' > CNAME
if [ -z "$GITHUB_TOKEN" ]; then
  msg='deploy'
  githubUrl=git@github.com:luoxiaofeng/blog.git
else
  msg='来自github action的自动部署'
  githubUrl=https://luoxiaofeng:${GITHUB_TOKEN}@github.com/luoxiaofeng/blog.git
  git config --global user.name "luoxiaofeng"
  git config --global user.email "954118124"
fi
git init
git add -A
git commit -m "${msg}"
...
if [ -z "$CODING_TOKEN" ]; then  # -z 字符串 长度为0则为true；$CODING_TOKEN来自于github仓库`Settings/Secrets`设置的私密环境变量
  codingUrl=git@git.dev.tencent.com:luoxiaofeng/luoxiaofeng.git
else
  codingUrl=https://luoxiaofeng:${CODING_TOKEN}@git.dev.tencent.com/luoxiaofeng/luoxiaofeng.git
fi
git add -A
git commit -m "${msg}"
...
<img src="https://cdn.jsdelivr.net/gh/xugaoyi/image_store/blog/20200103124813.png" class="imgcss" width="70%">
